...
import 'package:skillsync/resources/storage_methods.dart';
  ...
  Future<String> startLiveStream(
    ...
    try {
      if (title.isNotEmpty && image != null) {
        final docRef =
        _firestore.collection('livestream').doc('${user.user.uid}${user.user.username}');

        if (!(await docRef.get()).exists) {
          channelId = '${user.user.uid}${user.user.username}';

          // generate unique fileId for thumbnail
          String fileId = const Uuid().v1();
          String thumbnailUrl = await _storageMethods.uploadImageToStorage(
            'livestream-thumbnails',
            image,
            '$channelId/$fileId.jpg', // ✅ unique path
          );

          LiveStream liveStream = LiveStream(
            title: title,
            image: thumbnailUrl, // ✅ already full URL
            uid: user.user.uid,
          ...
          await docRef.set(liveStream.toMap());
        } else {
